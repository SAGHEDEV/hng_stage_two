...
// import MacBookPro14 from "../../../components/dev";

const HomePage = () => {
  const [page, setPage] = useState(1);
  const [currentDet, setCurrentDet] = useState();
  const [category, setCategory] = useState("54a6e5de079543a8b40a2959fbb157c1");
  const API = `https://api.timbu.cloud/products/?organization_id=${ORG_ID}&category_id=${category}&reverse_sort=true&page=${page}&size=6&Appid=${API_ID}&Apikey=${API_KEY}`;
  ...
  return (
    <div className="relative">
      ...
      <div className="px-5 md:px-[120px] py-8 grid grid-cols-2 lg:grid-cols-3 md:grid-cols-2 gap-5 lg:gap-10 blur-in2 relative">
        {prodLoad || currentProduct?.length === 0 || !prodData ? (
